...
#ifndef LB_SWAP // swap two value
#define LB_SWAP(_a_, _b_)  do { __typeof__(_a_) _tmp_ = (_a_); (_a_) = (_b_); (_b_) = _tmp_; } while (0)
#endif
...
- (UIImage *)imageByRoundCornerRadius2:(CGFloat)c {
...
+ (UIImage *)synthesisWithBottomImage:(UIImage *)btmImg topImage:(UIImage *)topImg withTopRect:(CGRect)rect{
    ...
    CGSize btmsize = btmImg.size;
    CGSize topsize = topImg.size;
    UIGraphicsBeginImageContext(CGSizeMake(btmsize.width, btmsize.height));
    [btmImg drawInRect:CGRectMake(0, 0, btmsize.width, btmsize.height)];
    if(CGRectEqualToRect(rect, CGRectZero)) {// 两个区域相等
        [topImg drawInRect:CGRectMake((btmsize.width - topsize.width) * 0.5, (btmsize.height - topsize.height) * 0.5, topsize.width, topsize.height)];
    }else{
...
- (UIImage*)addText:(NSString*)text
    ...
    char *txt = (char*)[text cStringUsingEncoding:NSUTF8StringEncoding];
    char *font = (char*)[@"PingFangSC-Semibold" cStringUsingEncoding:NSUTF8StringEncoding];
    
    CGContextSelectFont(context, font, (10), kCGEncodingMacRoman);
